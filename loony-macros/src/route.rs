...
impl Args {
    fn new(args: AttributeArgs) -> syn::Result<Self> {
        ...
        Ok(Args {
            ...
            guards,
            error: error
                .unwrap_or_else(|| syn::parse_str("loony::web::DefaultError").unwrap()),
        })
    }
}
...
impl Route {
    ...
    pub fn generate(&self) -> TokenStream {
        ...
        let stream = quote! {
            ...
            impl loony::web::dev::WebServiceFactory<#error> for #name
            {
                fn register(self, __config: &mut loony::web::dev::WebServiceConfig<#error>) {
                    #ast

                    let __resource = loony::web::Resource::new(#path)
                        .name(#resource_name)
                        .guard(loony::web::guard::#method())
                        #(.guard(loony::web::guard::fn_guard(#extra_guards)))*
                        .to(#name);

                    loony::web::dev::WebServiceFactory::register(__resource, __config)
                }
            }
        };
        stream.into()
