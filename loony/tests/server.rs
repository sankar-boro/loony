...
use loony::server::{Server, TestServer};
use loony::service::fn_service;
...
fn test_listen() {
    ...
    let h = thread::spawn(move || {
        ...
        let _ = tx.send(loony::rt::System::current());
        let _ = sys.run();
    });
...
fn test_panic_in_worker() {
    ...
    let h = thread::spawn(move || {
        ...
        let _ = tx.send((srv.clone(), loony::rt::System::current()));
        sys.exec(move || loony_rt::spawn(srv.map(|_| ())));
        let _ = sys.run();
    });
    let (_, sys) = rx.recv().unwrap();
