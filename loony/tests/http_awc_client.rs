...
use loony::http::client::error::{JsonPayloadError, SendRequestError};
use loony::http::client::{Client, Connector};
use loony::http::test::server as test_server;
use loony::http::{header, HttpMessage, HttpService};
use loony::service::{map_config, pipeline_factory, Service};
use loony::web::dev::AppConfig;
use loony::web::middleware::Compress;
use loony::web::{self, test, App, BodyEncoding, Error, HttpRequest, HttpResponse};
use loony::{time::Millis, time::Seconds, util::Bytes};
...
async fn test_timeout() {
    let srv = test::server(|| {
        App::new().service(web::resource("/").route(web::to(|| async {
            loony::time::sleep(Millis(2000)).await;
    ...
    let connector = Connector::default()
        .connector(
            loony::connect::Connector::new()
                .map(|sock| (sock, loony::http::Protocol::Http1)),
        )
        .timeout(Seconds(15))
        .finish();
...
async fn client_read_until_eof() {
    let addr = loony::server::TestServer::unused_addr();
    ...
    loony::time::sleep(Millis(300)).await;
