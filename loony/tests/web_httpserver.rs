...
use loony::{server::TestServer, time::Seconds};
...
async fn test_run() {
    ...
    thread::spawn(move || {
        let mut sys = loony::rt::System::new("test");
        ...
        let _ = tx.send((srv, loony::rt::System::current()));
        let _ = sys.run();
    });
    let (srv, sys) = rx.recv().unwrap();

    use loony::http::client;
...
#[cfg(feature = "openssl")]
fn client() -> loony::http::client::Client {
    ...
    loony::http::client::Client::build()
        .timeout(Seconds(30))
        .connector(
            loony::http::client::Connector::default()
                .timeout(Seconds(30))
    ...
    let client = client::Client::build()
        .connector(
            client::Connector::default()
                .connector(loony::service::fn_service(|_| async {
                    let stream =
                        loony::rt::net::UnixStream::connect("/tmp/uds-test").await?;
                    Ok((stream, loony::http::Protocol::Http1))
                }))
                .finish(),
        )
        .finish();
    ...
    let client = client::Client::build()
        .connector(
            client::Connector::default()
                .connector(loony::service::fn_service(|_| async {
                    let stream =
                        loony::rt::net::UnixStream::connect("/tmp/uds-test2").await?;
                    Ok((stream, loony::http::Protocol::Http1))
                }))
                .finish(),
        )
