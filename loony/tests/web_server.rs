...
use loony::http::body::Body;
use loony::http::header::{
    ContentEncoding, ACCEPT_ENCODING, CONTENT_ENCODING, CONTENT_LENGTH, CONTENT_TYPE,
    TRANSFER_ENCODING,
};
use loony::http::{Method, StatusCode};
use loony::time::{sleep, Millis, Seconds, Sleep};
...
async fn test_server_cookies() {
    use loony::http::header::SET_COOKIE;
...
async fn test_custom_error() {
    ...
    impl loony::web::ErrorContainer for JsonContainer {
        fn error_response(&self, req: &HttpRequest) -> HttpResponse {
            self.0.error_response(req)
        }
    }

    impl loony::http::ResponseError for JsonContainer {}
    ...
    impl loony::web::error::ErrorRenderer for JsonRenderer {
