    ...
    B: MessageBody + 'static,
{
    ...
    /// Stop loony runtime when server get dropped.
    ///
    ...
    fn listen_rustls_inner(
        ...
        self.builder = self.builder.listen(
            format!("loony-web-rustls-service-{}", addr),
            lst,
            move || {
                let c = cfg.lock().unwrap();
                let cfg = AppConfig::new(
    ...
    pub fn listen_uds(
        ...
        let addr = format!("loony-web-service-{:?}", lst.local_addr()?);
        ...
        A: AsRef<std::path::Path>,
    {
        ...
        self.builder = self.builder.bind_uds(
            format!("loony-web-service-{:?}", addr.as_ref()),
            addr,
            move || {
                let c = cfg.lock().unwrap();
                let config = AppConfig::new(
    ...
    B: MessageBody,
{
    ...
    /// `accept()` in a loop.
    ///
    /// This methods panics if no socket address can be bound or an loony system
